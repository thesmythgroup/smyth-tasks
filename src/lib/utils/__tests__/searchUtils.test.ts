...
describe("searchUtils", () => {
  ...
  describe("searchTasks", () => {
    ...
    it("should work with custom field configuration", () => {
      const tasksWithDescription = [
        { id: "1", title: "Task 1", description: "Go to grocery store" },
        { id: "2", title: "Task 2", description: "Walk the dog" },
      ];

      const result = searchTasks(tasksWithDescription, "grocery", {
        fields: ["description"],
      });
      expect(result).toHaveLength(1);
  ...
  describe("highlightText", () => {
    ...
    it("should highlight matching text (case insensitive)", () => {
      ...
      expect(result).toBe(
        'Hello <mark class="bg-yellow-400 text-gray-900 px-1 rounded font-semibold">world</mark>'
      );
    });
    ...
    it("should highlight first occurrence only", () => {
      ...
      expect(result).toBe(
        '<mark class="bg-yellow-400 text-gray-900 px-1 rounded font-semibold">Hello</mark> hello world'
      );
    });
  });
  ...
  describe("createSearchHighlighter", () => {
    it("should create a highlighter function with preset query", () => {
      ...
      expect(result).toBe(
        'This is a <mark class="bg-yellow-400 text-gray-900 px-1 rounded font-semibold">test</mark>'
      );
    });
